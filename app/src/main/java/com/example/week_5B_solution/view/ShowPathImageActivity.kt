    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (bundle!= null) {
            ...
            if (position != -1) {
                ...
                binding.editTextTitle.setText(MyPathAdapter.items[position].title)
                MyPathAdapter.items[position].description?.isNotEmpty().apply {
                    binding.editTextDescription.setText(MyPathAdapter.items[position].description)
                }
                try {
                    ...
                    Log.d(
                        "FileException",
                        "File Path: ${MyPathAdapter.items[position].imagePath} is Not Found"
                    )
                    e.printStackTrace()
                }
...
//                }
    ...
    private fun loadImageView(position: Int, defaultToPath: Boolean = true){
        val image_path = MyPathAdapter.items[position].imagePath
        if(defaultToPath){
    ...
    private fun onUpdateButtonClickListener(view: View, position: Int){
        ...
        MyPathAdapter.items[position].title = binding.editTextTitle.text.toString()
        MyPathAdapter.items[position].description = binding.editTextDescription.text.toString()

        this.appViewModel!!.updateImage(MyPathAdapter.items[position])

        runBlocking {
            launch(Dispatchers.IO) {

                // Start an intent to to tell the calling activity an update happened.
                val intent = Intent(this@ShowPathImageActivity, GalleryActivity::class.java)
    ...
    private fun onDeleteButtonClickListener(view: View, position: Int){
        ...
        this.appViewModel!!.deleteImage(MyPathAdapter.items[position])


        runBlocking {
            launch(Dispatchers.IO) {
                // Start intent and include data to let the calling activity know a deletion happened (include position payload
                val cacheFile = File(this@ShowPathImageActivity.cacheDir, MyPathAdapter.items[position].thumbnail)
                cacheFile.delete()
                MyPathAdapter.items.removeAt(position)
